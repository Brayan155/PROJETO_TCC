...
const concluidaBtn = document.querySelector('.marcar-concluidaBtn')
...
centralContent.addEventListener('click', () => {
    if (centralContent.style.backgroundColor === 'rgb(46, 70, 140)') {
        ...
        trashCanIcon.style.color = originalBackgrounColor
    } else {
...
//lixeira
const btnDeletarTarefa = document.querySelector('#btnDeletarTarefa')
btnDeletarTarefa.addEventListener('click', (event) => {
    event.stopPropagation(); // Impede a propagação para que o modal não seja aberto
    event.preventDefault(); // Cancela qualquer ação padrão do evento
    const tarefa = document.querySelector('#central-content');
    tarefa.remove(); // Remove a tarefa
});


function moverParaFeito() {
    if (!colunaFeito.contains(centralContent)) { // verifica se a tarefa já não está na coluna "Feito"
        colunaFeito.appendChild(centralContent);
        CircleCheck.style.color = 'green'; // marca a tarefa como concluída visualmente
    }
}

// Adiciona o evento ao botão "marcar como concluída"
concluidaBtn.addEventListener('click', () => {
    moverParaFeito(); // chama a função para mover a tarefa
});

let tarefaConcluida = false; // variável para acompanhar o estado de conclusão da tarefa

function marcarComoConcluida() {
    colunaFeito.appendChild(centralContent); // move para "Feito"
    CircleCheck.style.color = 'green'; // altera o estilo para indicar conclusão
    concluidaBtn.textContent = 'marcar como não concluída'; // altera o texto do botão
    tarefaConcluida = true;
}

function marcarComoNaoConcluida() {
    colunaAFazer.appendChild(centralContent); // move de volta para "A fazer"
    CircleCheck.style.color = ''; // restaura o estilo original
    concluidaBtn.textContent = 'marcar como concluída'; // altera o texto do botão
    tarefaConcluida = false;
}

// Evento do botão para alternar entre concluída e não concluída
concluidaBtn.addEventListener('click', () => {
    if (!tarefaConcluida) {
        marcarComoConcluida();
    } else {
