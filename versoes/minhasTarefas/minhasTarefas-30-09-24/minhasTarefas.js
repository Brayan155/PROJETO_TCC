...
// Evento para abrir o offcanvas quando o conteúdo da tarefa é clicado, mas não o ícone da lixeira
centralContent.addEventListener('click', (event) => {
    // Verifica se o alvo do clique não é o ícone da lixeira
    if (!event.target.closest('#trashCan1')) {
        // Abre o offcanvas se o clique não foi na lixeira
        const offcanvas = new bootstrap.Offcanvas(document.getElementById('offcanvasRight'));
        offcanvas.show();
    }
});

// Evento para remover o conteúdo ao clicar na lixeira
trashCanIcon.addEventListener('click', (event) => {
    event.preventDefault(); // Evita o comportamento padrão
    centralContent.remove(); // Remove o conteúdo da tarefa
});
